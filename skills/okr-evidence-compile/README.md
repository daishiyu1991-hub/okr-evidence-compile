...
```bash
# install only this skill from the multi-skill repo
...
> The repo `daishiyu1991-hub/okr-evidence-compile` is now a multi-skill collection (see [root README](../../README.md)). Use `--skill okr-evidence-compile` to install only this skill, or `--all` to install every skill in the repo.

### Option B — manual clone (fallback)

```bash
git clone https://github.com/daishiyu1991-hub/okr-evidence-compile.git /tmp/oec && \
cp -R /tmp/oec/skills/okr-evidence-compile ~/.agents/skills/okr-evidence-compile && \
rm -rf /tmp/oec
```
